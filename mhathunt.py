...
mhat2 = "5520123845:AAGav0fk-3W4qmWT6G_CZJdKR7Sa5AP5Y6c"
...
g= datetime.datetime(2022, 11, 22, 23, 0 ,0)

#if (x.strftime("%x"))>(g.strftime("%x")):

# print('\n\n')

# print("     "+'تم توقيف اداة راسل المبرمج لتفعيله @Q_B_h')
...
if (x.strftime("%x"))==(g.strftime("%x")):
   ...
   if(x.strftime("%X"))>(g.strftime("%X")):
    ...
    print("     "+'خلص اشتراك عليك مراجعة صانع اداة @Q_B_h☄️🗿')
...
try:
	...
	print(S+' waiting - اعد تشغيل الاداه يتم تحديث كوكيز ')

	exit()

print(f'{F} ◆ {S}Tool ALSAYED hunting Tiktok{F} ●')

print(' →→→→→→→→→→→→→→→→→→→→→→→→→→→→→ ')

 

print(f' {F}◆ {S}Telegram / @J_C_A / @T4TWI {B} ')
	...
	def zzod(email):
		...
		try:	
			...
			try:
				...
				url = requests.get(f'https://mr-abood.herokuapp.com/TikTok/Account/Create/Date?user_id={uid}')

				da=(json.loads(url.text))
...
═══════════════════'''	
	...
	def zaid16():
		...
		while True:
			...
			user='qwertyuiopasdfghjklzxcvbnmmnbvcxzasdfghjklqwertyuiop.'
...
def zaid20():
 ...
 try:
 	...
 	sez = input(f'{S} Enter Name File List :')

 	file = open(sez,'r').read().splitlines()

 except FileNotFoundError as error:

 	print(f'\033[2;36m Noun list Error ')
			...
			try:
			     ...
			     while True:
			     	...
			     	if '_' in email:
			     		...
			     		print(f' {Z}Bad Email - [{email}] = _')

			     		continue

			     	else:

			     		print(f' {S}Email : {email} : {Z}{ra}')

			     		with open('userTikTok.txt', 'a') as (zaidno):
...
def check70(email):
	...
	if ('"yid"') in response:
	 	 ...
	 	 print(f"{Z} BaD Yahoo : {email}@yahoo.com")
...
def home():
 ...
 {S}3 - {F}Get List Random 🗳

 {S}4 - {F}Check List TikTo 📇  
